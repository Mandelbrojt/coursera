...
# Recode the "female" variable
gender <- recode(cel$female, 
                 
                 # For each value = 1, replace it by "Female"
                 `1` = "Female", 
                 
                 # For each value = 0, replace it by "Male"
                 `0` = "Male")

# Replace the "female" variable with the recoded vector
cel <- cel %>% mutate(female = gender) %>% 
     
     # Rename the "female" variable to "Gender"
     rename("Gender" = "female")

# Filter the data used for the first plot
cel1 <- cel %>% filter(congress == 115)

ggplot(cel1, aes(x = dwnom1,
                y = all_pass,
                color = Gender)) + 
     geom_point() +
     xlab("Ideology") +
...
maj_min <- recode(cel1$majority,
                  
                  # For each value = 1, replace it by "Majority"
                  `1` = "Majority",
                  
                  # For each value = 0, replace it by "Minority"
                  `0` = "Minority")

cel2 <- cel1 %>% mutate(majority = maj_min) %>% 
        
        # Rename the "maj_min" variable
        rename("maj_min" = "majority")

ggplot(cel2, aes(x = votepct,
                 y = all_pass,
                 color = Gender)) +
        geom_point() + 
